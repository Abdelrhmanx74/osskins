...
pub async fn get_misc_items(app: AppHandle) -> Result<Vec<MiscItem>, String> {
  ...
  let misc_items: Vec<MiscItem> =
    serde_json::from_str(&content).map_err(|e| format!("Failed to parse misc items: {}", e))?;
...
pub async fn delete_misc_item(app: AppHandle, item_id: String) -> Result<(), String> {
  ...
  let content = fs::read_to_string(&misc_items_file)
    .map_err(|e| format!("Failed to read misc items file: {}", e))?;
  let mut misc_items: Vec<MiscItem> =
    serde_json::from_str(&content).map_err(|e| format!("Failed to parse misc items: {}", e))?;

  // Find and remove the item
  let item_to_remove = misc_items
    .iter()
...
#[tauri::command]
pub async fn upload_multiple_misc_items(
  app: AppHandle,
  item_type: String,
  ...
  println!(
    "Successfully uploaded {} misc items of type {}",
    uploaded_items.len(),
    item_type
  );
  Ok(uploaded_items)
...
pub fn get_selected_misc_items(app: &AppHandle) -> Result<Vec<MiscItem>, String> {
  ...
  let content = fs::read_to_string(&misc_items_file)
    .map_err(|e| format!("Failed to read misc items file: {}", e))?;
  let all_misc_items: Vec<MiscItem> =
    serde_json::from_str(&content).map_err(|e| format!("Failed to parse misc items: {}", e))?;
  ...
  println!(
    "DEBUG: Parsed config, selected_misc_items: {:?}",
  ...
  for (item_type, selected_ids) in &config.selected_misc_items {
    println!(
      "DEBUG: Processing type '{}' with {} selected IDs: {:?}",
      item_type,
      selected_ids.len(),
      selected_ids
    );
    for selected_id in selected_ids {
      ...
      for misc_item in &all_misc_items {
        if misc_item.item_type == *item_type && selected_ids.contains(&misc_item.id) {
          println!(
            "DEBUG: Adding selected item: {} ({})",
            misc_item.name, misc_item.id
          );
  ...
  println!(
    "DEBUG: Found {} selected misc items total",
    selected_items.len()
  );
  Ok(selected_items)
}

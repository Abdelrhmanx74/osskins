use crate::commands::types::{
  CustomSkinData, PartyModeConfig, SavedConfig, SkinData, ThemePreferences,
};
use serde_json;
use std::fs;
use std::path::Path;
...
pub async fn save_league_path(app: tauri::AppHandle, path: String) -> Result<(), String> {
  ...
  fs::write(&config_file, &path).map_err(|e| format!("Failed to write league path: {}", e))?;
...
pub async fn load_league_path(app: tauri::AppHandle) -> Result<String, String> {
  ...
  let path =
    fs::read_to_string(&config_file).map_err(|e| format!("Failed to read league path: {}", e))?;
...
pub async fn save_selected_skins(
  ...
  let mut config: serde_json::Value = if file.exists() {
    match std::fs::read_to_string(&file) {
      Ok(content) => match serde_json::from_str(&content) {
        ...
        Err(e) => {
          println!(
            "Warning: Failed to parse existing config.json, will overwrite: {}",
            e
          );
          serde_json::json!({})
        }
      ...
      Err(e) => {
        println!(
          "Warning: Failed to read config.json, will create new: {}",
          e
        );
        serde_json::json!({})
      }
  ...
  config["selected_misc_items"] =
    serde_json::to_value(selected_misc_items.unwrap_or_default()).unwrap();
...
pub async fn load_config(app: tauri::AppHandle) -> Result<SavedConfig, String> {
  ...
  let content = match std::fs::read_to_string(&file) {
    ...
    Err(e) => {
      println!(
        "Warning: Failed to read config.json: {}. Returning default config.",
        e
      );
      return Ok(SavedConfig {
        league_path: None,
  ...
  let mut cfg: SavedConfig = match serde_json::from_str(&content) {
    ...
    Err(e) => {
      println!(
        "Warning: Failed to parse config.json: {}. Returning default config.",
        e
      );
      return Ok(SavedConfig {
        league_path: None,

...
interface ToolsStore {
    ...
    setProgress: (source: ToolsSource, progress: ToolsProgressState | null) => void;
    mergeProgress: (
        source: ToolsSource,
    ...
    setStatus: (status: ToolsStatusState | null) => void;
...
export const useToolsStore = create<ToolsStore>((set, get) => ({
    ...
    mergeProgress: (source, partial) => {
        set((state) => {
            ...
            const nextProgress =
                typeof partial.progress === "number" ? partial.progress : previous.progress;
            const nextError = (() => {
            ...
            const next: ToolsProgressState = {
                ...
                version: partial.version ?? previous.version,
                error: nextError,
                updatedAt: Date.now(),
            };

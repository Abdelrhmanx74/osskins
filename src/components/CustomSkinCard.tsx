...
import { TrashIcon, Play, Check, MoreVertical, Pencil } from "lucide-react";
...
export function CustomSkinCard({ skin, onDelete, onRename }: CustomSkinCardProps) {
  const [isHovering, setIsHovering] = useState(false);
  const [imgLoaded, setImgLoaded] = useState(false);
  const [isDeleting, setIsDeleting] = useState(false);
  const manualInjectionMode = useGameStore((state) => state.manualInjectionMode);
  const customSelectedSkins = useGameStore((state) => state.customSelectedSkins);
  const manualCustomSelectedSkins = useGameStore((state) => state.manualCustomSelectedSkins);
  const addCustomSkinSelection = useGameStore((state) => state.addCustomSkinSelection);
  const removeCustomSkinSelection = useGameStore((state) => state.removeCustomSkinSelection);
  const addManualCustomSkinSelection = useGameStore((state) => state.addManualCustomSkinSelection);
  ...
  const isSelected = manualInjectionMode
    ? (manualCustomSelectedSkins.get(skin.champion_id) ?? []).some((s) => s.skin_file === skin.file_path)
    : (customSelectedSkins.get(skin.champion_id) ?? []).some((s) => s.skin_file === skin.file_path);
  ...
  return (
    <div
      ...
      tabIndex={0}
      className={cn(
        "w-full py-6 px-0 bg-primary/20 gap-0 rounded-lg overflow-hidden transition-all duration-300 flex items-center",

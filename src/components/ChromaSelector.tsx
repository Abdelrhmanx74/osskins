...
export function ChromaSelector({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "w-6 h-6 rounded-full border border-primary shadow cursor-pointer"
        )}
      ...
      >
        {chromas.map((chroma) => (
          <button
            key={chroma.id}
            className={cn(
              "w-6 h-6 rounded-full border flex items-center justify-center transition-all duration-200 bg-white/10 relative",

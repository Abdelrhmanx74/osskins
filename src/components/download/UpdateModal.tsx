...
interface UpdateModalProps {
    ...
    pill?: { label: string; sub?: string; loading?: boolean; badge?: string; badgeVariant?: "secondary" | "destructive" | "default" } | null;
        ...
        async function fetchCommitTitle() {
            ...
            try {
                ...
                if (typeof data === "object" && data !== null) {
                    ...
                    const commitObj = obj["commit"] as Record<string, unknown> | undefined;
                    const msg = commitObj && typeof commitObj["message"] === "string" ? commitObj["message"] : undefined;
                    if (msg) {
                ...
                <div className="size-full flex flex-col space-y-4">
                    ...
                    {showProgress && (
                        <div className="space-y-2">
                            ...
                            <div className="flex justify-between text-xs text-muted-foreground">
                                ...
                                <span>
                                    {progress.processedChampions ?? 0} of {progress.totalChampions ?? 0} champions
                                </span>
                            </div>
                            {progress.currentChampion && (
                                <p className="text-xs text-muted-foreground text-right">Currently: {progress.currentChampion}</p>
                            )}
                        </div>
                    )}
                        ...
                        <div className="rounded-md bg-muted/50 p-3 border border-border">
                            ...
                            <ul className="grid gap-1 pl-4 list-disc">
                                {updatedSkins.map((skin) => (
                                    <li key={skin} className="text-xs text-foreground">{skin}</li>
                                ))}
                            </ul>
                        </div>
                    )}
                    ...
                    {items && items.length > 0 && (
                        <div className="rounded-md bg-muted/50 p-3 border border-border">
                            ...
                            <p className="text-xs text-muted-foreground mb-3">New data updates are available for download.</p>
                            <div className="mb-3 space-y-1 text-xs text-muted-foreground">
                                <p className="font-medium text-foreground">Pending champions:</p>
                                <ul className="grid gap-1 pl-4 list-disc">

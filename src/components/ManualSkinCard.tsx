...
export const ManualSkinCard = memo(function ManualSkinCard({ championId, skin }: ManualSkinCardProps) {
  ...
  const handleChromaSelect = (chroma: CachedChroma | null) => {
    if (selectedChroma && chroma && selectedChroma.id === chroma.id) {
      ...
      selectManualSkin(championId, skin.id, undefined, skin.skin_file);
    } else {

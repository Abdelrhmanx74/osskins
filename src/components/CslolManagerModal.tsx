...
export default function CslolManagerModal({ isOpen, onClose }: Props) {
  ...
  const refresh = useCallback(async () => {
    ...
    setDone(false);
    try {
      const status = await invoke<CslolManagerStatus>(
  ...
  const install = useCallback(async () => {
    ...
    setDone(false);
    try {
      const res = await invoke<EnsureModToolsResult>("ensure_mod_tools", {
        force: true,
      });
      updateToolsStatus({
        ...
        hasUpdate: Boolean(
          res.latestVersion && res.version && res.version !== res.latestVersion
        ),
        path: res.path ?? null,
        lastChecked: Date.now(),
      });
  ...
  const primaryAction = {
    label: toolsStatus?.installed
      ? toolsStatus.hasUpdate
        ? "Update Tools"
        : "Reinstall Tools"
      : "Install Tools",
    onClick: install,
  ...
  const pillLabel = toolsStatus?.installed
    ? toolsStatus.version ?? "unknown"
    : t("tools.manager.not_installed");
  ...
  const pillBadge = toolsStatus?.installed
    ? toolsStatus.hasUpdate
      ? t("tools.manager.update_available")
      : t("tools.manager.up_to_date")
    : undefined;
  const pillBadgeVariant: "secondary" | "destructive" | "default" | undefined =
    ...
    <UpdateModal
      ...
      progress={
        showProgress
          ? {
              value: progressValue,
              processedChampions: undefined,
              totalChampions: undefined,
    ...
    >
      <div className="grid grid-cols-2 text-sm text-muted-foreground items-center justify-between">
        ...
        <div className="font-mono">
          {toolsStatus?.latestVersion ?? "unknown"}
        </div>
        {toolsStatus?.downloadSize ? (
          <>
            <div>Download size</div>
            <div>
              {(toolsStatus.downloadSize / (1024 * 1024)).toFixed(0)} MB
            </div>
          </>
        ) : null}
        <div>Location</div>
